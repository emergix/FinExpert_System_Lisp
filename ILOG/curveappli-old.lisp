...
(de curveappli (w1 h1 curves)
    ...
    (let* ((scale (:get-scale-bbox (get-slot-value (car curves) 'body)
           ...
           (user (#:user:repeater " Disp "))
           (user1 (#:user:repeater " Fonct "))
           (user2 (#:user:repeater " Reset "))
           (user3 (#:user:repeater " Trans "))
      ...
      (set-action user2 '(lambda (c eve) 
                           (prog (courbe liste-droite)
                                 (setq courbe (car (get-slot-value ({application}:expert (component 'appli c))
                                 ...
                                 (setf (get-slot-value ({application}:expert (component 'appli c))
                                                       'graphique-liste-des-objects-affiches)
                                       (cons courbe liste-droite))
                                 (setf (get-slot-value ({application}:expert (component 'appli c))
                                                               'representation-graphique-transitoire) nil)
                                 (display-courbe-a (cons courbe liste-droite))
                                 (full-redisplay  (get-slot-value ({application}:expert (component 'appli c)) 'graphique))
                                 )))
                                 
                                 
      (set-action user3 '(lambda (c eve) (setf (get-slot-value ({application}:expert (component 'appli c))
